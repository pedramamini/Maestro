...
- **Shell escape:** `shellEscape()` in `src/main/utils/shell-escape.ts`
...
- **Session lookup:** `selectSessionById()` in `src/renderer/stores/sessionStore.ts`
- **Modal layer:** `useModalLayer()` in `src/renderer/hooks/ui/useModalLayer.ts`

The dedup tracker at [DEDUP-TRACKER.md](docs/agent-guides/DEDUP-TRACKER.md) lists all known duplication with grep-verified evidence in `docs/agent-guides/scans/`.

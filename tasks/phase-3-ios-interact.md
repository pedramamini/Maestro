...
- [x] Provide example flows for common scenarios
  - Note: Created 10 comprehensive example flow files in `docs/examples/ios-flows/` covering: login-flow.yaml (authentication with env vars), onboarding-flow.yaml (multi-screen carousel navigation), search-flow.yaml (search input and results), form-validation-flow.yaml (input validation and error states), settings-navigation-flow.yaml (hierarchy navigation with toggles), shopping-cart-flow.yaml (e-commerce checkout with promo codes), pull-to-refresh-flow.yaml (refresh gestures), deep-link-flow.yaml (URL schemes and universal links), photo-picker-flow.yaml (image selection and upload), logout-flow.yaml (session cleanup). Added README.md with usage guide, customization tips, and troubleshooting. Added "Example Flows" section to docs/ios-development.md with inline code examples, quick start guide, and best practices summary.

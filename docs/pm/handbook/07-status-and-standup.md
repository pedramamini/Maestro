...
Always query local PM / Work Graph state. Use `/PM status`, `/PM standup`, `/PM next`, or from a shell `{{MAESTRO_CLI_PATH}} fleet board --project <path> --json`. Never reconstruct status from conversation history, markdown files alone, external tracker labels, GitHub issues, or project boards.

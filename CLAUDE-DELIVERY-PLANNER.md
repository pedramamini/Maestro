...
2. **GitHub sync is explicit, fork-only.** All issue and PR operations target the configured repository (default: `RunMaestro/Maestro`, override via `MAESTRO_GITHUB_OWNER`/`MAESTRO_GITHUB_REPO`). Operations targeting other repos are rejected.
...
└── deliveryPlannerRoutes.ts    # DeliveryPlannerRoutes class (Fastify)
```
...
All GitHub issue operations target the configured repository (default: `RunMaestro/Maestro`). This is enforced before any `gh` CLI invocation by `assertDeliveryPlannerGithubRepository` in `github-safety.ts`.

### Fork Safety Constants

```typescript
import {
	DELIVERY_PLANNER_GITHUB_REPOSITORY, // resolved from MAESTRO_GITHUB_OWNER/MAESTRO_GITHUB_REPO env
...
Synced items set the following fields on `Humpf Tech Maestro Features` (project #7, configured owner):
...
The Delivery Planner web surface is implemented in `DeliveryPlannerRoutes` (`src/main/web-server/routes/deliveryPlannerRoutes.ts`). All routes are prefixed with a security token: `/<token>/api/…`.
...
Delivery Planner registers the following slash commands (`src/renderer/slashCommands.ts`, `ccpmSlashCommands`). All are `aiOnly: true`.

| Command           | Description                                           |
| ----------------- | ----------------------------------------------------- |
| `/ccpm prd`       | Draft a Delivery Planner PRD from the current request |
| `/ccpm decompose` | Decompose a Delivery Planner PRD or epic into tasks   |
| `/ccpm sync`      | Sync Delivery Planner items to CCPM mirror files      |
| `/ccpm next`      | Show the next ready Delivery Planner item             |
| `/ccpm status`    | Show Delivery Planner status for this project         |
| `/ccpm bug`       | Draft a Delivery Planner bug follow-up                |

Three of these (`/ccpm prd`, `/ccpm decompose`, `/ccpm bug`) are also wired as `ccpmPromptCommands` — built-in prompt templates with `$ARGUMENTS` substitution that agents receive when the command is invoked.
...
A sync call targeted a repository other than the configured target. This is a programming error — check that no `gh` call is passing a `-R` value derived from user input without validation. Using `DeliveryPlannerGithubSync` without overrides always targets the fork.
...
4. **GitHub sync:** call `syncGithubIssue` on a task → one issue appears on the configured target repo, `WorkItem.github.issueNumber` is written back, Project fields are populated, and not on any other repo.
...
| ---------------------------------------------- | ----------------------------------------------------- | --------------------------------------------------------------- |
| 001 — Contract alignment                       | [#60](https://github.com/RunMaestro/Maestro/issues/60) | `.claude/epics/delivery-planner/001-contract-alignment.md`      |
| 002 — CCPM path resolver + mirror              | [#61](https://github.com/RunMaestro/Maestro/issues/61) | `.claude/epics/delivery-planner/002-ccpm-mirror.md`             |
| 003 — Main-process services                    | [#62](https://github.com/RunMaestro/Maestro/issues/62) | `.claude/epics/delivery-planner/003-main-services.md`           |
| 004 — Desktop IPC + preload                    | [#63](https://github.com/RunMaestro/Maestro/issues/63) | `.claude/epics/delivery-planner/004-desktop-api.md`             |
| 005 — PRD wizard                               | [#64](https://github.com/RunMaestro/Maestro/issues/64) | `.claude/epics/delivery-planner/005-prd-wizard-desktop.md`      |
| 006 — Decomposition workflow                   | [#65](https://github.com/RunMaestro/Maestro/issues/65) | `.claude/epics/delivery-planner/006-decomposition-workflow.md`  |
| 007 — Desktop dashboard                        | [#66](https://github.com/RunMaestro/Maestro/issues/66) | `.claude/epics/delivery-planner/007-desktop-dashboard.md`       |
| 008 — GitHub sync                              | [#67](https://github.com/RunMaestro/Maestro/issues/67) | `.claude/epics/delivery-planner/008-github-sync.md`             |
| 009 — Slash commands + bridges                 | [#68](https://github.com/RunMaestro/Maestro/issues/68) | `.claude/epics/delivery-planner/009-slash-and-bridges.md`       |
| 010 — Web/mobile surface                       | [#69](https://github.com/RunMaestro/Maestro/issues/69) | `.claude/epics/delivery-planner/010-web-mobile.md`              |
| 011 — Living Wiki + Agent Dispatch integration | [#70](https://github.com/RunMaestro/Maestro/issues/70) | `.claude/epics/delivery-planner/011-cross-major-integration.md` |
| 012 — Tests, docs, validation                  | [#71](https://github.com/RunMaestro/Maestro/issues/71) | `.claude/epics/delivery-planner/012-tests-docs-validation.md`   |

See [PRD](https://github.com/RunMaestro/Maestro/issues/59) and `.claude/prds/delivery-planner.md` for full product requirements.

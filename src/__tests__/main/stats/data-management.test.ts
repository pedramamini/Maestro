...
describe('Database VACUUM functionality', () => {
	beforeEach(() => {
		...
		// Reset stat to reject by default (simulates file not existing)
		mockFsStat.mockImplementation(() => Promise.reject(new Error('ENOENT: no such file or directory')));
	});
	...
	describe('getDatabaseSize', () => {
		it('should return 0 when stat rejects (file missing)', async () => {
			// The mock fs.promises.stat rejects by default
			// so getDatabaseSize will catch the error and return 0
			const { StatsDB } = await import('../../../main/stats');
			const db = new StatsDB();
			await db.initialize();

			// getDatabaseSize will try to call fs.promises.stat on a non-existent path
		...
		it('should handle stat gracefully when file does not exist', async () => {
			...
			await expect(db.getDatabaseSize()).resolves.not.toThrow();
		});
	});
	...
	describe('initialize with vacuumIfNeeded integration', () => {
		it('should call vacuumIfNeededWeekly during initialization', async () => {
			...
			mockFsReadFile.mockImplementation(() => Promise.resolve('0'));

...
describe('useMarketplace', () => {
  ...
  describe('initial manifest loading', () => {
    ...
    it('handles thrown exceptions gracefully', async () => {
      vi.mocked(window.maestro.marketplace.getManifest).mockRejectedValue(new Error('Unexpected error'));
  ...
  describe('import playbook functionality', () => {
    ...
    it('handles thrown exceptions during import', async () => {
      vi.mocked(window.maestro.marketplace.importPlaybook).mockRejectedValue(new Error('Network error'));
  ...
  describe('document fetching', () => {
    describe('fetchReadme', () => {
      ...
      it('returns null on exception', async () => {
        vi.mocked(window.maestro.marketplace.getReadme).mockRejectedValue(new Error('Network error'));
    ...
    describe('fetchDocument', () => {
      it('returns content when successful', async () => {
        ...
        expect(window.maestro.marketplace.getDocument).toHaveBeenCalledWith('playbooks/test', 'phase-1');
      });
      ...
      it('returns null on exception', async () => {
        vi.mocked(window.maestro.marketplace.getDocument).mockRejectedValue(new Error('Network error'));

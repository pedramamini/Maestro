...
function createMockExternalNodes(count: number): Node<ExternalLinkNodeData>[] {
  const domains = ['github.com', 'stackoverflow.com', 'npmjs.com', 'docs.example.com', 'api.example.com'];
  return Array.from({ length: count }, (_, i) => ({
...
describe('React Flow Performance Profiling: 200+ Nodes', () => {
  ...
  describe('Viewport Culling Performance', () => {
    ...
    it('documents visible node calculation at different zoom levels', () => {
      ...
      const visibleAtZoom1 = Math.ceil(viewportWidth / nodeWidth) * Math.ceil(viewportHeight / nodeHeight);
      const visibleAtZoom05 = Math.ceil((viewportWidth / 0.5) / nodeWidth) * Math.ceil((viewportHeight / 0.5) / nodeHeight);
    ...
    it('estimates render time with vs without culling for 200 nodes', () => {
      ...
      const timeWithoutCulling = nodesTotal * msPerNode + nodesTotal * edgesPerNode * msPerEdge + 100;
      const timeWithCulling = nodesVisible * msPerNode + nodesVisible * edgesPerNode * msPerEdge + 20;
  ...
  describe('Stress Test Scenarios', () => {
    ...
    it('estimates initial render time for 250 nodes', () => {
      ...
      const initialRenderTime = layoutTime + injectionTime + reconciliationTime + domCreationTime + edgeTime;
  ...
  describe('Performance Thresholds Summary', () => {
    it('summarizes target performance metrics', () => {
      const performanceTargets = {
        ...
        cachedRender200Nodes: { target: 400, unit: 'ms', description: 'Subsequent render with cache' },

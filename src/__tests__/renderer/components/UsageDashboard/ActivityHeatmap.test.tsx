 ...
 * - Locale-aware chart labels via Intl.DateTimeFormat (I18N-15)
 */
...
describe('ActivityHeatmap', () => {
	...
	describe('Locale-Aware Chart Labels (I18N-15)', () => {
		it('renders UI strings via i18n (not hardcoded English)', () => {
			render(<ActivityHeatmap data={mockData} timeRange="week" theme={theme} />);

			// These resolve from common:chart.* keys via the test mock
			expect(screen.getByText('Activity Heatmap')).toBeInTheDocument();
			expect(screen.getByText('Count')).toBeInTheDocument();
			expect(screen.getByText('Duration')).toBeInTheDocument();
			expect(screen.getByText('Less')).toBeInTheDocument();
		...
		it('renders "Show:" label via i18n', () => {
			render(<ActivityHeatmap data={mockData} timeRange="week" theme={theme} />);

			expect(screen.getByText('Show:')).toBeInTheDocument();
		});

		it('generates locale-correct day-of-week labels for year view', () => {
			render(<ActivityHeatmap data={mockData} timeRange="year" theme={theme} />);

			// In the GitHub-style grid, Mon/Wed/Fri are shown (odd indices)
			// Intl.DateTimeFormat('en', { weekday: 'short' }) produces "Mon", "Wed", "Fri"
			expect(screen.getByText('Mon')).toBeInTheDocument();
			expect(screen.getByText('Wed')).toBeInTheDocument();
			expect(screen.getByText('Fri')).toBeInTheDocument();
		});

		it('generates locale-correct hour labels for week view', () => {
			const { container } = render(
				<ActivityHeatmap data={mockData} timeRange="week" theme={theme} />
			);

			// Hour labels are generated by Intl.DateTimeFormat('en', { hour: 'numeric' })
			// Even-hour labels are shown (idx % 2 === 0), so 12 AM, 2 AM, etc.
			// Check that at least some Intl-formatted hour labels exist
			const hourLabelElements = container.querySelectorAll('.text-xs.text-right');
			expect(hourLabelElements.length).toBeGreaterThan(0);
		});

		it('generates locale-correct time block labels for month view', () => {
			const { container } = render(
				<ActivityHeatmap data={mockData} timeRange="month" theme={theme} />
			);

			// Time block labels should be rendered on Y-axis
			// Intl generates labels like "12 AM–4 AM" (with en-dash) in English
			const yAxisLabels = container.querySelectorAll('.text-xs.text-right');
			expect(yAxisLabels.length).toBeGreaterThan(0);
		});

		it('uses Intl.DateTimeFormat for tooltip date display', () => {
			const { container } = render(
				<ActivityHeatmap data={mockData} timeRange="week" theme={theme} />
			);
			...
			if (cells.length > 0) {
				...
				// Tooltip should contain a locale-formatted date (not raw date-fns format)
				// The Intl formatter produces locale-appropriate date strings
				if (tooltip) {

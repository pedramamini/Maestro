...
import { DeliveryPlannerGithubSafetyError, DELIVERY_PLANNER_GITHUB_OWNER, DELIVERY_PLANNER_GITHUB_REPO, DELIVERY_PLANNER_GITHUB_REPOSITORY } from '../github-safety';
...
describe('DeliveryPlannerGithubSync', () => {
	it('creates fork-local issues, adds them to the project, and sets organization fields', async () => {
		...
		const exec = vi.fn(async (_command: string, args: string[]) => {
			...
			if (args[0] === 'issue' && args[1] === 'create') {
				return ok(`https://github.com/${DELIVERY_PLANNER_GITHUB_OWNER}/${DELIVERY_PLANNER_GITHUB_REPO}/issues/67\n`);
			}
			if (args[0] === 'project' && args[1] === 'view') {
				return ok(JSON.stringify({ id: 'project-7', title: DELIVERY_PLANNER_GITHUB_PROJECT_TITLE }));
			}
	...
	it('reuses existing GitHub issue references instead of creating duplicates', async () => {
		const exec = vi.fn(async (_command: string, args: string[]) => {
			if (args[0] === 'issue' && args[1] === 'view') {
				return ok(
					JSON.stringify(`{ "number": 67, "url": "https://github.com/${DELIVERY_PLANNER_GITHUB_OWNER}/${DELIVERY_PLANNER_GITHUB_REPO}/issues/67" }`)
				);
			}
			if (args[0] === 'project' && args[1] === 'view') {
				return ok(JSON.stringify({ id: 'project-7', title: DELIVERY_PLANNER_GITHUB_PROJECT_TITLE }));
		...
		const result = await sync.syncIssue(
			makeWorkItem({
				github: {
					...
					repository: DELIVERY_PLANNER_GITHUB_REPOSITORY,
					issueNumber: 67,
					url: `https://github.com/${DELIVERY_PLANNER_GITHUB_OWNER}/${DELIVERY_PLANNER_GITHUB_REPO}/issues/67`,
				},
			})
		);
			...
			sync.syncIssue(
				makeWorkItem({
					github: {
						owner: DELIVERY_PLANNER_GITHUB_OWNER,
						repo: 'Maestro',
						repository: 'RunMaestro/Maestro' as DELIVERY_PLANNER_GITHUB_REPOSITORY,
						issueNumber: 1,
					},
				})
			)
	...
	it('maps status and progress comments to fork-local issue commands', async () => {
		...
		expect(exec).toHaveBeenCalledWith(
			'gh',
			['issue', 'comment', '67', '-R', DELIVERY_PLANNER_GITHUB_REPOSITORY, '--body', 'Implementation complete.'],
			undefined
		);
		expect(exec).toHaveBeenCalledWith(
			'gh',
			expect.arrayContaining(['issue', 'close', '67', '-R', DELIVERY_PLANNER_GITHUB_REPOSITORY]),
			undefined
		);
	});
});

...
export function registerHistoryHandlers(): void {
		...
		withIpcErrorLogging(
			...
			async (options?: {
				...
				if (sessionId) {
					...
					return await historyManager.getEntriesPaginated(sessionId, pagination);
				}
				...
				return await historyManager.getAllEntriesPaginated(pagination);
			}
		)
	);
	...
	ipcMain.handle(
		...
		withIpcErrorLogging(handlerOpts('add'), async (entry: HistoryEntry) => {
			...
			await historyManager.addEntry(sessionId, entry.projectPath, entry);

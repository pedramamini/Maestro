 ...
 * groupChatId is ALWAYS a uuidv4() — the regex patterns in constants.ts enforce
 * this so a participant name containing "-participant-" (or any other sentinel)
 * can no longer cause a mis-parse that routes output to the wrong owner.
 *
 * Examples:
...
export function parseParticipantSessionId(
	...
	if (uuidMatch) {
		const participantName = uuidMatch[2].replace(/-recovery$/, '');
		return { groupChatId: uuidMatch[1], participantName };
	}

...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    // Windows: hide the menu bar by default (press Alt to show)
...
if (process.platform === 'win32') {
  app.setAppUserModelId('ai.runmaestro.app');
}
...
if (!gotTheLock) {
  logger.info('Another instance is already running. Quitting...', 'Startup');
  app.quit();
} else {
  app.on('second-instance', (_event, _commandLine, _workingDirectory) => {
    ...
    // Optional: Handle protocol links or command line args from second instance here
    logger.info('Second instance detected and refocused', 'Window');
  });

  // Proceed with normal app startup inside the lock
      ...
      async (
        ...
        try {
          ...
          if (response.ok) {
            ...
            return {
              success: true, entries: data.entries as Array<{
                rank: number;
                displayName: string;
                githubUsername?: string;
                avatarUrl?: string;

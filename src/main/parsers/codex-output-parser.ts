 ...
 */
function extractErrorText(error: CodexOldMessage['error'], fallback = 'Unknown error'): string {
...
export class CodexOutputParser implements AgentOutputParser {
	...
	parseJsonObject(parsed: unknown): ParsedEvent | null {
		...
		return this.routeMessage(parsed as Record<string, unknown>);
	}
	...
	detectErrorFromLine(line: string): AgentError | null {
		...
		return this.detectErrorFromParsedText(errorText, parsedJson);
	}

	/**
	 * Detect error from a pre-parsed JSON object.
	 * Required by AgentOutputParser interface.
	 */
	detectErrorFromParsed(parsed: unknown): AgentError | null {
		...
		// New format: check inside msg envelope
		if (!errorText && obj.msg && typeof obj.msg === 'object') {
		...
		return this.detectErrorFromParsedText(errorText, parsed);
	}

	/**
	 * Shared error detection logic for both detectErrorFromLine and detectErrorFromParsed.
	 */
	private detectErrorFromParsedText(errorText: string, parsedJson: unknown): AgentError | null {
